...
public class bowAndArrow : MonoBehaviour {
	...
	void Start () {
		...
		keywords.Add("forte", Forte);
		keywords.Add("puxa", Forte);
		keywords.Add("poxa", Forte);
		keywords.Add("pucha", Forte);
        
        keywords.Add("fraco", Fraco);
		keywords.Add("desse", Fraco);

        keywords.Add("cima", Cima);
		keywords.Add("up", Cima);
		keywords.Add("sobe", Cima);
		keywords.Add("só que", Cima);
		keywords.Add("chove", Cima);

        keywords.Add("baixo", Baixo);
        keywords.Add("down", Baixo);

		keywords.Add("sol", Solta);
        keywords.Add("solta", Solta);
		keywords.Add("solte", Solta);
		keywords.Add("release", Solta);

		keywords.Add("enter", Enter);
		keywords.Add("inter", Enter);
		keywords.Add("entre", Enter);
		keywords.Add("sair", Exit);
    ...
    private void DictationRecognizer_OnDictationResult(string text, ConfidenceLevel confidence)
    {
        ...
        if (isUserSpeaking == true)
        {
            ...
            int pos = System.Array.IndexOf(keywords.Keys.ToArray(), text);

            if(pos >= 0){
                keywords[text].Invoke();
            }
        }
    }
	...
	public void prepareArrow() {
		...
		if (Physics.Raycast(mouseRay1, out rayHit, 1000f) && arrowShot == false)
		{
			...
			// flag--
			// bow angle
			// angleZ = Mathf.Atan2(mousePos.y,mousePos.x)*Mathf.Rad2Deg;
